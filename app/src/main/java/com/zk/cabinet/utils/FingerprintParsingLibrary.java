...
public class FingerprintParsingLibrary {
    ...
    public void init(Context context) {
        ...
        LogUtil.Companion.getInstance().d("指纹open ：fingerprintSensor：：：" + fingerprintSensor);

        try {
            fingerprintSensor.open(0);
        } catch (FingerprintSensorException e) {
            e.printStackTrace();
            LogUtil.Companion.getInstance().d("指纹open ：：：：" + e);
        }
        fingerprintSensor.setFingerprintCaptureListener(0, new FingerprintCaptureListener() {
            ...
            public void captureOK(int i, byte[] bytes, int[] ints, byte[] bytes1) {
                LogUtil.Companion.getInstance().d("指纹：i：：：" + i);
                ...
                LogUtil.Companion.getInstance().d("指纹FingerprintCaptureListener", "test Received" + buffers);
                ...
                if (isVerify) {
                    ...
                    for (User user : userList) {
                        if (user.getFingerPrint() != null) {
                            ...
                            LogUtil.Companion.getInstance().d("指纹对比结果：" + result);
                            if (result >= 23) {
            ...
            public void captureError(FingerprintSensorException e) {
                LogUtil.Companion.getInstance().d("error：FingerprintSensorException：：：" + e);
            }
        });
        try {
            ...
            e.printStackTrace();
            LogUtil.Companion.getInstance().d("指纹：startCapture FingerprintSensorException：：：" + e);
        }
...
//        fingerprintService = new FingerprintService();
        int[] limit = new int[1];
        LogUtil.Companion.getInstance().d("指纹对比初始化结果：" + FingerprintService.init(limit));
    }
...
//                        Toast.makeText(context, "USB未授权", Toast.LENGTH_SHORT).show();
                        LogUtil.Companion.getInstance().d("USB未授权");
