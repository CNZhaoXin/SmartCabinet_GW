...
import com.zk.cabinet.adapter.ZNGReturnAdapter
...
import com.zk.cabinet.databinding.ActivityZngReturnBinding
 ...
 * 档案单柜-归还
 */
class ZNGReturnActivity : TimeOffAppCompatActivity(), AdapterView.OnItemClickListener,
    View.OnClickListener {
    private lateinit var mBinding: ActivityZngReturnBinding
    ...
    companion object {
        ...
        fun newIntent(packageContext: Context?): Intent {
            return Intent(packageContext, ZNGReturnActivity::class.java)
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mBinding =
            DataBindingUtil.setContentView(this, R.layout.activity_zng_return)
    ...
    private lateinit var mAdapter: ZNGReturnAdapter
    private var mList = ArrayList<ResultGetToReturnList.DataBean>()

    private fun initAdapter() {
        mAdapter = ZNGReturnAdapter(this, mList)
                        ...
                        if (200 == code) {
                            ...
                            if (mList.size > 0) {
                                ...
                                if (curCabinetDossier.size > 0) {
                                    showWarningToast("该柜有「${curCabinetDossier.size}」份档案需归还")
                                    speek("该柜有${curCabinetDossier.size}份档案需归还")
    ...
    private fun handleMessage(msg: Message) {
        when (msg.what) {
            GET_INFRARED_AND_LOCK -> {
                ...
                Log.e("zx-档案单柜归还-boxStateList", "$boxStateList")
                Log.e("zx-档案单柜归还-infraredState", "$infraredStateList")
                ...
                if (boxStateList!!.isEmpty()) {
                    isAutoFinish = true
                    timerStart()
                    Log.e("zx-档案单柜待归还", "门关闭-开启界面倒计时")
                    showWarningToast("柜门已关闭,请点击归还,若已归还请忽略")
                    speek("柜门已关闭,请点击归还,若已归还请忽略")

                    // 柜门关闭后,若还有可归还的档案,开柜按钮才可用
                    var isExist = false
                    for (entity in mList) {
                        if (entity.cabinetEquipmentId == mDevice!!.deviceId) {
                            isExist = true
            ...
            OPEN_DOOR_RESULT -> {
                Log.e("zx-档案单柜待归还", "门开启-关闭界面倒计时-归还的档案进行开灯")
                mProgressDialog.dismiss()
                // 柜门已开启
                ...
                for (floor in 1..5) {
                    ...
                    if (lights.size > 0) {
                        ...
                        LogUtils.e("档案单柜-归还-亮灯-亮的灯:", JSON.toJSONString(lights))
                    }
                }
                ...
                LogUtils.e("档案单柜-归还-亮大灯-亮大灯的柜子:", mDevice!!.deviceId)
                ...
                if (lightControlRecords != null && lightControlRecords.size > 0) {
                    ...
                    LogUtils.e("档案单柜-归还-该柜亮灯亮大灯记录已存在,无需再添加")
                } else {
                    // 不存在,新增一条某柜亮大灯记录
                    val bigLightRecord = LightControlRecord(null, mDevice!!.deviceId, "", 1)
                    LightControlRecordService.getInstance().insert(bigLightRecord)

                    LogUtils.e("档案单柜-归还-新增该柜亮大亮记录", JSON.toJSONString(bigLightRecord))
                }

                // 亮灯记录打印
                val allRecords = LightControlRecordService.getInstance().loadAll()
                if (allRecords != null && allRecords.size > 0) {
                    LogUtils.e("档案单柜-归还-亮灯记录:", allRecords.size, JSON.toJSONString(allRecords))
                } else {
                    LogUtils.e("档案单柜-归还-亮灯记录: null")
                }
            }
            ...
            INVENTORY_VALUE -> {
                ...
                Log.e("档案单柜待归还", "labelInfo.deviceID: ${labelInfo.deviceID}")
                Log.e("档案单柜待归还", "labelInfo.antennaNumber: ${labelInfo.antennaNumber}")
                Log.e("档案单柜待归还", "labelInfo.fastID: ${labelInfo.fastID}")
                Log.e("档案单柜待归还", "labelInfo.rssi: ${labelInfo.rssi}")
                Log.e("档案单柜待归还", "labelInfo.operatingTime: ${labelInfo.operatingTime}")
                Log.e("档案单柜待归还", "labelInfo.epcLength: ${labelInfo.epcLength}")
                Log.e("档案单柜待归还", "labelInfo.epc: ${labelInfo.epc}")
                Log.e("档案单柜待归还", "labelInfo.tid: ${labelInfo.tid}")
                Log.e("档案单柜待归还", "labelInfo.inventoryNumber: ${labelInfo.inventoryNumber}")
            }
            ...
            END_INVENTORY -> {
                ...
                Log.e("档案单柜待归还-盘点到的标签数据-", JSON.toJSONString(labelInfoList))
                mProgressDialog.dismiss()
                mAdapter.notifyDataSetChanged()
                labelInfoList.clear()
            }
    ...
    private class MyHandler(activity: ZNGReturnActivity) :
        Handler() {
    ...
    private val mCabinetInfoListener = object : CabinetInfoListener {
        ...
        override fun unlockResult(p0: Int) {
            val message = Message.obtain()
            message.obj = "柜门已开启,库位已亮灯,请放入待归还的档案"
