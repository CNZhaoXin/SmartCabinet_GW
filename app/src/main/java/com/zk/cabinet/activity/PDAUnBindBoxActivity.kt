...
import com.zk.cabinet.databinding.ActivityPdaUnbindBoxBinding
import com.zk.cabinet.entity.RequestUnBind
...
// 消息类型: 档案盒解绑
private const val UNBIND_SUCCESS = 0x01
private const val UNBIND_ERROR = 0x02

/**
 * PDA-档案盒解绑库位
 */
class PDAUnBindBoxActivity : TimeOffAppCompatActivity(), View.OnClickListener {
    private lateinit var mBinding: ActivityPdaUnbindBoxBinding
    ...
    companion object {
        fun newIntent(packageContext: Context?): Intent {
            return Intent(packageContext, PDAUnBindBoxActivity::class.java)
        }
    }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        mBinding = DataBindingUtil.setContentView(this, R.layout.activity_pda_unbind_box)
    ...
    private fun unBind() {
        ...
        mProgressDialog.setMessage("正在解绑库位...")
        mProgressDialog.show()

        val requestUrl = NetworkRequest.instance.mUnBind
        LogUtils.e("档案盒解绑库位-请求URL:$requestUrl")

        val requestUnBind = RequestUnBind()
        requestUnBind.dataValue = boxId

        val jsonObject = JSONObject(JSON.toJSONString(requestUnBind))
        LogUtils.e("档案盒解绑库位-请求参数:" + JSON.toJSONString(requestUnBind))
        LogUtils.e("档案盒解绑库位-请求参数:$jsonObject")

        val jsonObjectRequest = JsonObjectRequestWithHeader(
            Request.Method.POST, requestUrl, jsonObject,
            { response ->
                LogUtils.e("档案盒解绑库位-请求结果:", "$response")
                try {
                    if (response != null) {
                        val code = response.getInt("code")
                        if (200 == code) {
                            sendDelayMessage(UNBIND_SUCCESS, "档案盒解绑库位成功")
                            mBinding.tvBoxId.text = ""
                        } else {
                            sendDelayMessage(UNBIND_ERROR, response.getString("msg"))
                        }
                    } else {
                        sendDelayMessage(UNBIND_ERROR, "档案盒解绑库位-失败")
                    }
                } catch (e: JSONException) {
                    e.printStackTrace()
                    sendDelayMessage(UNBIND_ERROR, "档案盒解绑库位-失败")
    ...
    private class MyHandler(activity: PDAUnBindBoxActivity) : Handler() {
